...
class ResultActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val myHand: Int
        myHand = when(id) {
            R.id.gu -> { myHandImage.setImageResource(R.drawable.gu); gu }
            R.id.choki -> { myHandImage.setImageResource(R.drawable.choki); choki }
            R.id.pa -> { myHandImage.setImageResource(R.drawable.pa); pa }
            else -> gu
        }
    ...
    private fun saveData(myHand: Int, comHand: Int, gameResult: Int){
        ...
        editor.putInt("GAME_COUNT", gameCount+1)
            ...
            .putInt("LAST_COM_HAND", lastComHand)
    ...
    private fun getHand():Int{
        ...
        if (gameCount==1){
            if(gameResult==2){              //前回の勝負が一回目で、cpが勝った場合、cpは次に出す手を変える
                while (lastComHand==hand){
                    hand=(Math.random()*3).toInt()
                }
            }else if (gameResult==1){       //前回の勝負が一回目で、cpが負けた場合、相手の出した手に勝つ手を出す
                hand=(lastMyHand-1+3)%3
            }
        }else if (winningStreakCount > 0){
            if (beforeLastComHand==lastComHand){    //同じ手で連勝した場合、手を変える
                while(lastComHand==hand){
                    hand=(Math.random()*3).toInt()
                }
            }
